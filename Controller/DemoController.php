...
use Orca\UserLogBundle\DataTable;


use Orca\UserLogBundle\Chart\PieChart;
...
class DemoController extends Controller
{
    public function demoAction()
    {
        ...
        return $this->render(
            'OrcaUserLogBundle:Demo:demo.html.twig',
            array(
                'dt' => $dt->toStrictArray(),
    ...
    public function demo2Action()
    {
        ...
        $dt->addRows($myArray);
        return $this->render(
            'OrcaUserLogBundle:Demo:demo2.html.twig',
            array(
                'dt' => $dt->toStrictArray()
            )
        );
    ...
    public function demo3Action()
    {
        ...
        var_dump($dataTable);
        return $this->render(
            'OrcaUserLogBundle:Demo:demo3.html.twig',
            array(
    ...
    public function demoTypeDeviceAction()
    {
        $date = new \DateTime('now');

        $em = $this->getDoctrine()->getManager();
        $repository = $em->getRepository('OrcaUserLogBundle:TblUserLog');
        $queryJour = $repository->createQueryBuilder('p')
                        ->where('p.date  > CURRENT_DATE()')
                        ->where('p.errorCode in (200,500)')
                        ->getQuery()
                        ->getResult();
        ...
        $query = $repository->createQueryBuilder('p')
                        ->where('p.date > :d1')
                        ->andWhere('p.date < :d2')
                        ->setParameter('d1', $date->format('Y').'-'.$date->format('m').'-01 00:00:00')
                        ->setParameter('d2', $date->format('Y').'-'.$date->format('m').'-31 23:59:59')
                        ->getQuery();
//var_dump($query);die;
        //how to deal with 0 values #2
        /*
        ...
        return $this->render(
            'OrcaUserLogBundle:Demo:demoTypeDevice.html.twig',
            array(
                'dataTable1' => $dataTable1->toArray(),
                'rawDataTable1' => $dataTable1,
                'queryJour' => count($queryJour)
            )
        );
    }
}
